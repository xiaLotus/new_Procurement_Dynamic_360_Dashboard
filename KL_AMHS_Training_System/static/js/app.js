...
// const API_BASE = 'http://10.11.104.247:9031';
...
const vueApp = Vue.createApp({
  ...
  data() {
    return {
      ...
      securityCat: 'all',            // [Security C] 分類篩選
      securitySearch: '',            // [Security C] 關鍵字篩選
    };
  },
...
apiFetch(path, opts){
  ...
  return fetch(API_BASE + path, opts).then(res=>{
    if(res.status===401){
      ...
      location.href='KL_AMHS_Training_System.html';
  ...
  try{
    ...
    if(res.ok){
      ...
      this.state.employees.forEach(emp=>{
        ...
        if(!Array.isArray(emp.security)) emp.security = [];  // [Security C] 伺服器已用範本補齊,此為保險
...
renderOnboarding(){
  // 頁籤「認證資訊」:只呈現 Security C(KL_Security_C.csv)的認證資料。
  // 原「新人權限與報到項目追蹤」清單已移除(資料仍保留在 emp.onboarding,不再顯示)。
  const el = document.getElementById('tab-onboarding');
  const myEmpId = (this.currentUser && this.currentUser.empId || '').toUpperCase();

  // 【檢視權限】所有人都可以互相查看;勾選 / 新增 / 移除限主管。
  const emps = this.state.employees.slice().sort((a,b)=>(a.empId||'').localeCompare(b.empId||''));
  if(!emps.length){
    el.innerHTML = `<div class="empty fade-up"><div class="empty-icon">🔐</div><div>目前尚無人員資料</div><div style="font-size:12px;margin-top:8px;color:var(--text4)">請主管至「訓練紀錄表」新增人員後再查看</div></div>`;
    return;
  }
  if(!this.onboardingSelectedEmpId || !emps.find(e => e.empId === this.onboardingSelectedEmpId)){
    const mine = emps.find(e => (e.empId||'').toUpperCase() === myEmpId);
    this.onboardingSelectedEmpId = (mine || emps[0]).empId;
  }
  const emp = emps.find(e => e.empId === this.onboardingSelectedEmpId);
  const isSelf = (emp.empId||'').toUpperCase() === myEmpId;

  el.innerHTML = `<div class="fade-up">
    <h2 style="font-size:16px;font-weight:700;margin-bottom:14px">🔐 認證資訊</h2>
    <div class="flex items-center gap-12 mb-16 fw-wrap">
      <label class="label" style="margin:0">選擇人員</label>
      <select class="field field-sm" style="width:240px" onchange="_vm.onboardingSelectedEmpId=this.value;renderOnboarding()">
        ${emps.map(e => `<option value="${esc(e.empId)}" ${e.empId===this.onboardingSelectedEmpId?'selected':''}>${esc(e.empId)} — ${esc(e.name)||'未命名'}${(e.empId||'').toUpperCase()===myEmpId?'（本人）':''}</option>`).join('')}
      </select>
      ${isSelf ? '<span class="badge" style="background:var(--blue)20;color:var(--blue2);border:1px solid var(--blue)50">本人</span>' : ''}
      <span class="badge" style="background:var(--surface2);color:var(--text2);border:1px solid var(--border2)">組別:${esc(emp.group)||'未設定'}</span>
    </div>
    <div id="securityRadar" class="mb-16"></div>
    <div id="securityBox"></div>
  </div>`;
  this.renderSecurityRadar(emp);
  this.renderSecurityBox(emp);
  this._saveView();  // 記錄選中的人員,供刷新後還原
},

// ─── [Security C] 分類戰鬥圖(雷達圖)───────────────────────────────────
// 依「分類」統計具備百分比(具備數 / 該分類項目數),另加一軸「認證項目」(標記為認證項目的具備率),
// 三個分類 + 認證項目 = 四邊形;分類數不同時多邊形自動調整。勾選是否具備後即時重繪。
getSecurityCatStats(emp){
  const items = emp.security || [];
  const order = ['系統', 'AMHS', '中控系統'];   // 固定順序,其餘分類接在後面
  ...
  const stats = cats.map(c => {
    const list = items.filter(i => (i.category||'未分類') === c);
    const has = list.filter(i => i.has).length;
    return {label:c, has, total:list.length, pct: list.length ? Math.round(has/list.length*100) : 0};
  });
  const certList = items.filter(i => i.isCert);
  const certHas  = certList.filter(i => i.has).length;
  stats.push({label:'認證項目', has:certHas, total:certList.length, pct: certList.length ? Math.round(certHas/certList.length*100) : 0, isCert:true});
  return stats;
},

renderSecurityRadar(emp){
  const box = document.getElementById('securityRadar');
  if(!box || !emp) return;
  const stats = this.getSecurityCatStats(emp);
  const n = stats.length;
  if(n < 3){ box.innerHTML = ''; return; }

  const cx=200, cy=165, R=105;
  const accent = '#a78bfa';
  const colors = ['#0ea5e9','#f59e0b','#22d3ee','#ef4444','#10b981','#f472b6'];
  const angOf = i => -90 + i*360/n;                 // 第一軸在正上方,順時針
  const rad = a => a*Math.PI/180;
  const pt = (a,r) => [ cx + r*Math.cos(rad(a)), cy + r*Math.sin(rad(a)) ];
  const P = (x,y)=>x.toFixed(1)+','+y.toFixed(1);

  let svg = `<svg width="400" height="330" viewBox="0 0 400 330" xmlns="http://www.w3.org/2000/svg" style="overflow:visible;max-width:100%">`;
  // 網格環(25/50/75/100%)
  [25,50,75,100].forEach(lvl=>{
    const r = R*lvl/100;
    const poly = stats.map((_,i)=>{ const [x,y]=pt(angOf(i),r); return P(x,y); }).join(' ');
    svg += `<polygon points="${poly}" fill="none" stroke="${lvl===100?'#37445f':'#2a3550'}" stroke-width="1"/>`;
    const [tx,ty]=pt(-90,r);
    svg += `<text x="${tx+6}" y="${ty+4}" font-size="9" fill="#64748b">${lvl}%</text>`;
  });
  // 軸線
  stats.forEach((_,i)=>{ const [x,y]=pt(angOf(i),R); svg += `<line x1="${cx}" y1="${cy}" x2="${x.toFixed(1)}" y2="${y.toFixed(1)}" stroke="#2a3550" stroke-width="1"/>`; });
  // 資料多邊形
  const dpoly = stats.map((st,i)=>{ const [x,y]=pt(angOf(i), R*st.pct/100); return P(x,y); }).join(' ');
  svg += `<polygon points="${dpoly}" fill="${accent}33" stroke="${accent}" stroke-width="2" stroke-linejoin="round" style="transition:all .3s"/>`;
  stats.forEach((st,i)=>{
    const [x,y]=pt(angOf(i), R*st.pct/100);
    svg += `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="4" fill="${colors[i%colors.length]}" stroke="#0f172a" stroke-width="1.5"/>`;
  });
  // 角落標籤:分類名稱 + 百分比
  stats.forEach((st,i)=>{
    const a = angOf(i);
    ...
    const dy = Math.sin(rad(a)) < -0.2 ? -4 : (Math.sin(rad(a)) > 0.2 ? 14 : 4);
    svg += `<text x="${x.toFixed(1)}" y="${(y+dy).toFixed(1)}" text-anchor="${anchor}" font-size="12" font-weight="600" fill="#cbd5e1">${esc(st.label)}</text>`;
    svg += `<text x="${x.toFixed(1)}" y="${(y+dy+15).toFixed(1)}" text-anchor="${anchor}" font-size="14" font-weight="800" fill="${colors[i%colors.length]}">${st.pct}%</text>`;
  });
  svg += `</svg>`;

  const legend = stats.map((st,i)=>`
    <div class="flex items-center gap-8" style="padding:6px 0;border-bottom:1px solid var(--border)">
      ...
      <span style="font-weight:600;min-width:80px">${esc(st.label)}</span>
      <div style="flex:1;height:6px;background:var(--surface2);border-radius:3px;overflow:hidden"><div style="width:${st.pct}%;height:100%;background:${colors[i%colors.length]};transition:width .3s"></div></div>
      <span style="font-family:var(--mono);font-size:12px;color:var(--text2);min-width:70px;text-align:right">${st.has} / ${st.total}</span>
      <span style="font-weight:800;color:${colors[i%colors.length]};min-width:44px;text-align:right">${st.pct}%</span>
    </div>`).join('');

  box.innerHTML = `
    <div class="card" style="border-color:${accent}30">
      <div class="card-head" style="background:${accent}08">
        <span style="font-size:14px;font-weight:700">📊 分類具備率戰鬥圖</span>
        <span class="badge ml-auto" style="background:var(--surface2);color:var(--text3)">${esc(emp.name)||''}（${esc(emp.empId)}）</span>
      </div>
      <div class="flex items-center gap-16 fw-wrap" style="padding:12px 16px">
        <div style="flex:0 0 auto">${svg}</div>
        <div style="flex:1;min-width:260px">${legend}</div>
      </div>
    </div>`;
},

// ─── [Security C] 系統權限清單(KL_Security_C.csv)───────────────────────
// 資料來源:emp.security(伺服器由 data/KL_Security_C.csv 產生範本後補給每位員工)。
// 姓名 / 工號直接取員工資料,主要操作為「是否具備」勾選;主管可新增 / 移除項目(該員或全員)。
renderSecurityBox(emp){
  const box = document.getElementById('securityBox');
  if(!box || !emp) return;
  const amLeader = this.isLeader();
  const items = emp.security || [];
  const cats  = [...new Set(items.map(i => i.category || '未分類'))];
  if(this.securityCat !== 'all' && this.securityCat !== '__cert' && !cats.includes(this.securityCat)) this.securityCat = 'all';

  const hasCount  = items.filter(i => i.has).length;
  const certTotal = items.filter(i => i.isCert).length;
  const certHas   = items.filter(i => i.isCert && i.has).length;
  const pct = items.length ? Math.round(hasCount / items.length * 100) : 0;

  box.innerHTML = `
    <div class="card" style="border-color:var(--purple,#8b5cf6)30">
      <div class="card-head" style="background:var(--purple,#8b5cf6)08;flex-wrap:wrap;gap:8px">
        <span style="font-size:14px;font-weight:700">🔐 Security C 系統權限清單</span>
        <span class="badge" style="background:var(--surface2);color:var(--text);border:1px solid var(--border2);font-weight:700">${esc(emp.name)||'未命名'}（${esc(emp.empId)}）</span>
        <span class="badge" style="background:var(--surface2);color:var(--text3)">認證項目 ${certHas} / ${certTotal}</span>
        <div class="ml-auto flex items-center gap-10" style="font-size:13px;color:var(--text2)">
          ...
          <div style="width:150px;height:8px;background:var(--surface2);border-radius:4px;overflow:hidden">
            <div style="width:${pct}%;height:100%;background:var(--purple,#8b5cf6);transition:width .3s"></div>
          </div>
          <span style="font-weight:700;color:var(--purple,#8b5cf6)">${hasCount} / ${items.length}</span>
        </div>
      </div>
      <div class="flex items-center gap-8 fw-wrap" style="padding:10px 14px;border-bottom:1px solid var(--border)">
        <button class="btn btn-ghost btn-xs" style="${this.securityCat==='all'?'border-color:var(--purple,#8b5cf6);color:var(--purple,#8b5cf6)':''}" onclick="_vm.securityCat='all';renderSecurityTable('${emp.id}')">全部（${items.length}）</button>
        <button class="btn btn-ghost btn-xs" style="${this.securityCat==='__cert'?'border-color:#f59e0b;color:#f59e0b':''}" onclick="_vm.securityCat='__cert';renderSecurityTable('${emp.id}')">只看認證項目（${certTotal}）</button>
        ${cats.map(c => `<button class="btn btn-ghost btn-xs" style="${this.securityCat===c?'border-color:var(--purple,#8b5cf6);color:var(--purple,#8b5cf6)':''}" onclick="_vm.securityCat='${esc(c)}';renderSecurityTable('${emp.id}')">${esc(c)}（${items.filter(i=>(i.category||'未分類')===c).length}）</button>`).join('')}
        <div class="search-bar ml-auto" style="min-width:220px">
          <span class="search-icon">🔍</span>
          <input class="field field-sm" id="securitySearch" placeholder="搜尋項目 / 子項目 / 說明..." value="${esc(this.securitySearch)}"
            oninput="_vm.securitySearch=this.value.trim();renderSecurityTable('${emp.id}')">
        </div>
        ${amLeader ? `<button class="btn btn-primary btn-xs" onclick="openAddSecurityItem('${emp.id}')">＋ 新增項目</button>` : ''}
      </div>
      <div id="securityTableWrap" style="overflow-x:auto"></div>
    </div>`;
  this.renderSecurityTable(emp.id);
},

renderSecurityTable(empId){
  const wrap = document.getElementById('securityTableWrap');
  const emp  = this.state.employees.find(e => e.id === empId);
  if(!wrap || !emp) return;
  const amLeader = this.isLeader();
  const q = (this.securitySearch || '').toLowerCase();
  const items = (emp.security || []).filter(i => {
    if(this.securityCat === '__cert'){ if(!i.isCert) return false; }
    else if(this.securityCat !== 'all' && (i.category || '未分類') !== this.securityCat) return false;
    if(!q) return true;
    return [i.item, i.subItem, i.desc, i.cert, i.category].some(v => String(v||'').toLowerCase().includes(q));
  });

  if(!items.length){
    wrap.innerHTML = `<div class="empty"><div class="empty-icon">🔐</div><div>沒有符合的項目</div>${amLeader?'<div style="font-size:12px;margin-top:8px;color:var(--text4)">可按「＋ 新增項目」加入</div>':''}</div>`;
    return;
  }

  // 依「分類 › 項目」自然分組:同一項目連續列時只在第一列顯示名稱
  let prevKey = null;
  let html = `<table class="tbl" style="min-width:980px">
    <thead><tr>
      <th style="width:70px;text-align:center">是否具備</th>
      <th style="width:60px;text-align:center">認證</th>
      <th style="width:90px">分類</th>
      <th style="min-width:180px">項目</th>
      <th style="min-width:200px">子項目</th>
      <th style="min-width:220px">功能說明</th>
      <th style="width:140px">廠商/工程師認證</th>
      ${amLeader ? '<th style="width:110px;text-align:center">操作</th>' : ''}
    </tr></thead><tbody>`;

  items.forEach(it => {
    const key = (it.category||'') + '|' + (it.item||'');
    const first = key !== prevKey; prevKey = key;
    const certSel = amLeader
      ? `<select class="field field-sm" onchange="updateSecurityField('${emp.id}','${it.id}','cert',this.value)">
           <option value="" ${!it.cert?'selected':''}>—</option>
           <option value="盟立廠商認證" ${it.cert==='盟立廠商認證'?'selected':''}>盟立廠商認證</option>
           <option value="工程師認證" ${it.cert==='工程師認證'?'selected':''}>工程師認證</option>
           ${it.cert && !['盟立廠商認證','工程師認證'].includes(it.cert) ? `<option value="${esc(it.cert)}" selected>${esc(it.cert)}</option>` : ''}
         </select>`
      : (it.cert ? `<span class="badge" style="background:var(--surface2);color:var(--text2);border:1px solid var(--border2)">${esc(it.cert)}</span>` : '<span style="color:var(--text4)">—</span>');
    html += `<tr style="${first?'border-top:2px solid var(--border2)':''}">
      <td style="text-align:center">
        <input type="checkbox" style="width:20px;height:20px;accent-color:var(--purple,#8b5cf6);${amLeader?'cursor:pointer':'cursor:not-allowed;opacity:.5'}"
          ${it.has?'checked':''} ${amLeader?'':'disabled'} title="${amLeader?'勾選 = 具備':'僅主管可勾選'}"
          onchange="updateSecurityField('${emp.id}','${it.id}','has',this.checked)">
      </td>
      <td style="text-align:center">${it.isCert ? '<span class="badge" style="background:#f59e0b20;color:#f59e0b;border:1px solid #f59e0b50">認證</span>' : ''}</td>
      <td style="color:var(--text3)">${first ? esc(it.category||'未分類') : ''}</td>
      <td style="font-weight:600;${it.has?'color:var(--text4)':''}">${first ? esc(it.item) : ''}</td>
      <td style="white-space:pre-line;${it.has?'color:var(--text4)':''}">${esc(it.subItem)}</td>
      <td style="font-size:12px;color:var(--text2);white-space:pre-line">${esc(it.desc)}</td>
      <td>${certSel}</td>
      ${amLeader ? `<td style="text-align:center;white-space:nowrap">
        <button class="btn btn-ghost btn-xs" title="只從此人移除" onclick="removeSecurityItem('${emp.id}','${it.id}','one')">移除</button>
        <button class="btn btn-danger btn-xs" title="從全員與範本移除" onclick="removeSecurityItem('${emp.id}','${it.id}','all')">全員</button>
      </td>` : ''}
    </tr>`;
  });
  html += `</tbody></table>`;
...
updateSecurityField(empId, itemId, field, value){
  const emp = this.state.employees.find(e => e.id === empId);
  const it  = emp && (emp.security || []).find(i => i.id === itemId);
  if(!it) return;
  it[field] = value;
  this._afterPatch(
    this.apiFetch('/api/employee/'+emp.empId+'/security/'+itemId, {
      method:'PATCH', body: JSON.stringify({[field]: value})
    }), emp);
  if(field === 'has'){ this.renderSecurityRadar(emp); this.renderSecurityBox(emp); }   // 即時更新戰鬥圖、進度與列樣式
},

openAddSecurityItem(empId){
  const emp = this.state.employees.find(e => e.id === empId);
  if(!emp) return;
  const cats = [...new Set(this.state.employees.flatMap(e => (e.security||[]).map(i => i.category)).filter(Boolean))];
  const root = document.getElementById('modalRoot');
  root.innerHTML = `<div class="overlay" onclick="if(event.target===this)closeModal()">
    <div class="modal" style="max-width:640px">
      <div class="modal-head"><span style="font-weight:700;font-size:15px">🔐 新增 Security 項目</span><button class="btn btn-ghost btn-xs" onclick="closeModal()">✕</button></div>
      <div class="modal-body">
        <div class="mb-16">
          <label class="label">套用範圍 <span style="color:var(--red)">*</span></label>
          <div class="flex gap-12 fw-wrap">
            <label class="flex items-center gap-8" style="cursor:pointer"><input type="radio" name="secScope" value="one" checked> 僅 ${esc(emp.name)||''}（${esc(emp.empId)}）</label>
            <label class="flex items-center gap-8" style="cursor:pointer"><input type="radio" name="secScope" value="all"> 全員（同時寫入範本，之後新人也會有）</label>
          </div>
        </div>
        <div class="grid g2 mb-16">
          <div><label class="label">分類</label><input class="field" id="secCat" list="secCatList" placeholder="例: 系統 / AMHS / 中控系統"><datalist id="secCatList">${cats.map(c=>`<option value="${esc(c)}">`).join('')}</datalist></div>
          <div><label class="label">項目 <span style="color:var(--red)">*</span></label><input class="field" id="secItem" placeholder="例: MES CENTER WEB REPORT"></div>
        </div>
        <div class="mb-16"><label class="label">子項目</label><input class="field" id="secSub" placeholder="例: A3自動化By Pass設定"></div>
        <div class="mb-16"><label class="label">功能說明</label><textarea class="field" id="secDesc" rows="2" placeholder="功能說明"></textarea></div>
        <div class="grid g2">
          <div><label class="label">廠商或工程師認證</label>
            <select class="field" id="secCert"><option value="">—</option><option>盟立廠商認證</option><option>工程師認證</option></select></div>
          <div><label class="label">&nbsp;</label>
            <label class="flex items-center gap-8" style="cursor:pointer;height:38px"><input type="checkbox" id="secIsCert"> 列為認證項目</label></div>
        </div>
      </div>
      <div class="modal-foot"><button class="btn btn-ghost btn-sm" onclick="closeModal()">取消</button><button class="btn btn-primary btn-sm" onclick="confirmAddSecurityItem('${emp.id}')">確認新增</button></div>
    </div>
  </div>`;
  setTimeout(()=>{ const el=document.getElementById('secItem'); if(el) el.focus(); }, 0);
},

async confirmAddSecurityItem(empId){
  const emp = this.state.employees.find(e => e.id === empId);
  if(!emp) return;
  const body = {
    category: document.getElementById('secCat').value.trim(),
    item:     document.getElementById('secItem').value.trim(),
    subItem:  document.getElementById('secSub').value.trim(),
    desc:     document.getElementById('secDesc').value.trim(),
    cert:     document.getElementById('secCert').value,
    isCert:   document.getElementById('secIsCert').checked,
  };
  if(!body.item){ alert('請填寫「項目」名稱'); return; }
  const scope = (document.querySelector('input[name="secScope"]:checked')||{}).value || 'one';
  const url = scope === 'all' ? '/api/security/items' : '/api/employee/'+emp.empId+'/security/items';
  try{
    const res = await this.apiFetch(url, {method:'POST', body: JSON.stringify(body)});
    const d = await res.json().catch(()=>null);
    if(!res.ok || !d || !d.item){ this._markPatchFailed(d && d.error); return; }
    if(scope === 'all'){
      this.state.employees.forEach(e => {
        if(!Array.isArray(e.security)) e.security = [];
        if(!e.security.find(i => i.id === d.item.id)) e.security.push(JSON.parse(JSON.stringify(d.item)));
        this['_snap_'+e.id] = JSON.stringify(e);   // 伺服器已寫入,更新快照避免整包重送
      });
      this.showSaveToast('已新增至全員（'+d.affected+' 人）');
    } else {
      if(!Array.isArray(emp.security)) emp.security = [];
      emp.security.push(d.item);
      this['_snap_'+emp.id] = JSON.stringify(emp);
      this.showSaveToast('已新增項目');
    }
    this.closeModal();
    this.renderSecurityRadar(emp);
    this.renderSecurityBox(emp);
  }catch(e){
    console.error('add security item failed:', e);
    this._markPatchFailed('新增失敗');
  }
},

async removeSecurityItem(empId, itemId, scope){
  const emp = this.state.employees.find(e => e.id === empId);
  const it  = emp && (emp.security || []).find(i => i.id === itemId);
  if(!it) return;
  const label = [it.category, it.item, it.subItem].filter(Boolean).join(' / ');
  const msg = scope === 'all'
    ? `確定從「全員」與範本移除此項目？\n\n${label}\n\n所有人員的此項目與勾選狀態都會被刪除。`
    : `確定只從 ${emp.name||''}（${emp.empId}）移除此項目？\n\n${label}`;
  if(!confirm(msg)) return;
  const url = scope === 'all' ? '/api/security/items/'+itemId : '/api/employee/'+emp.empId+'/security/'+itemId;
  try{
    const res = await this.apiFetch(url, {method:'DELETE'});
    if(!res || !res.ok){
      const d = await res.json().catch(()=>null);
      this._markPatchFailed(d && d.error || '移除失敗'); return;
    }
    const targets = scope === 'all' ? this.state.employees : [emp];
    targets.forEach(e => {
      e.security = (e.security || []).filter(i => i.id !== itemId);
      this['_snap_'+e.id] = JSON.stringify(e);
    });
    this.showSaveToast(scope === 'all' ? '已從全員移除' : '已移除項目');
    this.renderSecurityRadar(emp);
    this.renderSecurityBox(emp);
  }catch(e){
    console.error('remove security item failed:', e);
    this._markPatchFailed('移除失敗');
  }
},
...
exportData(){
  ...
  a.href = url;
  a.download = `KL_AMHS_Training_${new Date().toISOString().slice(0,10)}.json`;
  a.click();
...
// ─── INIT ───
  ...
  mounted() {
    ...
    if (!token || !user) { location.href = 'KL_AMHS_Training_System.html'; return; }
