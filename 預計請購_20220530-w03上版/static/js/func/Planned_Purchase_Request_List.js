const app = Vue.createApp({
    ...
    computed: {
            ...
            return Array.from(new Set(
                baseData
                    .filter(i => {
                        ...
                        && matchPONo && matchItem &&　matchText && matchAmount && matchStage && matchStatus && matchRemark;
                    })
                    .map(i => i['需求原因'] || '')
                    .filter(Boolean)
            )).sort();
            ...
            return Array.from(new Set(
                baseData
                    .filter(i => {
                        ...
                        return matchReceivingResult && matchPerson && matchState && matchWBS && matchOrder && matchNeedDate && matchIssuedMonth && matchEPR && matchPONo 
                        && matchItem &&　matchText && matchReason && matchStage && matchStatus && matchRemark;
                    })
        ...
        issuedMonthOptions() {
            ...
            const clean = str => String(str || '').replace(/\.0+$/, '').trim();
        ...
        monthlyTotalAmount() {
            ...
            return this.items.reduce((sum, item) => {
                ...
                if (monthKey === month && !wbsPattern.test(wbs)) {
    ...
    applyFilters(filters) {
        ...
        async saveEdit() {
            if (this.editingIndex !== null) {
                ...
                const previousStatus = this.items[this.editingIndex]['開單狀態'];
                const currentStatus = updated['開單狀態'];
                console.log("開單前狀態: ", previousStatus, ", 開單後狀態: ", currentStatus)
                ...
                if (res.ok) {
                    this.items.splice(this.editingIndex, 1, updated); // 🔄 替換資料讓 Vue 重算
                    this.cancelEdit();
                    ...
                    if (issuedDate.length >= 7) {
                        this.selectedIssuedMonth = issuedDate.slice(0, 4) + issuedDate.slice(5, 7); 
                    }
                    ...
                    const updatedMonth = updated['需求日'].slice(0, 6);

                    const totalAfterUpdate = this.items.reduce((sum, item) => {
                        const rawDate = clean(item['已開單日期']);
                        const itemMonth = rawDate.length >= 6 ? rawDate.slice(0, 6) : '';

                        const wbs = clean(item['WBS']);
                        const rawAmount = clean(item['總金額']).replace(/,/g, '');
                        const amount = parseFloat(rawAmount);

                        if (updatedMonth === itemMonth && !wbsPattern.test(wbs)) {
                            return sum + (isNaN(amount) ? 0 : amount);
                        }

                        return sum;
                    }, 0)
                    console.log("更新後累積金額:", totalAfterUpdate);

                    // 判斷提示
                    if (totalAfterUpdate >= 1000000) {
                        this.showColoredAlert("❌ 超過 100 萬元，已超出管制線", 'red');
                    } else if (totalAfterUpdate >= 900000) {
                        this.showColoredAlert("⚠️ 已達 90% 管制線，請注意審核", 'orange');
                    } else {
        ...
        async saveNewItem() {
            ...
            if ((this.newItem["請購項目"] || "").trim() === "" && (this.newItem["需求者"] || "").trim() === "") {
                alert("請購項目 以及 需求者 不允許為空");
                return;
            }
            ...
            const wbsPattern = /^\d{2}FT0A\d{4}$/;  // WBS
            const wbs = (this.newItem['WBS'] || "").trim();

            const amountPattern = /^\d+$/;
            const amount = String(this.newItem['總金額'] || "").trim().replace(/[^0-9]/g, '');
            this.newItem['總金額'] = amount;
            console.log(this.newItem['總金額'])
            ...
            const payload = {
                ...this.newItem,           // 原本的請購主資料
                tableRows: this.yourTableData  // 表格資料傳入後端
            };
            ...
            try {
                ...
                if (resp.status === 'success') {
                    await this.fetchData(); // 加這行重新載入 CSV
                    ...
                    if(this.newItem['開單狀態'] === "V"){
                        ...
                        const updatedMonth = this.newItem['需求日'].slice(0, 6);
        ...
        async copyToNewItem(){
            ...
            // 清空指定欄位的值（保留欄位，但清掉內容）
            const fieldsToClear = ['PO No.', 'User', 'ePR No.', '交貨驗證', '開單狀態'];
